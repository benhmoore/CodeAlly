...
class PermissionManager:
    ...
    def __init__(self, trust_manager: TrustManager):
        ...
        logger.info(f"PermissionManager initialized with starting directory: {self.start_directory}")
    ...
    def _initialize_path_restrictions(self) -> None:
        ...
        self.path_traversal_patterns = [
            r'\.\.', r'\.\.\/', r'~\/', r'\$HOME', 
            r'\$\{HOME\}', r'\$\(pwd\)', r'`pwd`',
            r'\/etc\/', r'\/var\/', r'\/usr\/', r'\/bin\/', r'\/tmp\/',
            r'\/root\/', r'\/proc\/', r'\/sys\/', r'\/dev\/',
            r'\/\*', r'~\/\*'
        ]
        
        # Compile regex patterns for efficiency
        self.path_traversal_regexes = [re.compile(pattern) for pattern in self.path_traversal_patterns]
    ...
    def _check_all_arguments_for_traversal(self, tool_name: str, arguments: Dict[str, Any]) -> None:
        ...
        for arg_name, arg_value in arguments.items():
            if isinstance(arg_value, str):
                ...
                if has_path_traversal_patterns(arg_value):
                    logger.warning(f"Path traversal pattern detected in {tool_name} argument {arg_name}: {arg_value}")
                ...
                if "/" in arg_value or "\\" in arg_value or "." in arg_value:
                    try:
                        ...
                        if not abs_path.startswith(self.start_directory):
                            logger.warning(f"Path outside CWD detected in {tool_name} argument {arg_name}: {arg_value}")
                            raise DirectoryTraversalError(
                        ...
                        logger.debug(f"Could not validate potential path in {tool_name} argument {arg_name}: {e}")
            
            # Check string arrays recursively
            elif isinstance(arg_value, list):
                for item in arg_value:
                    if isinstance(item, str) and has_path_traversal_patterns(item):
                        logger.warning(f"Path traversal pattern detected in {tool_name} list argument {arg_name}: {item}")
                        raise DirectoryTraversalError(
            ...
            # Check nested dictionaries recursively
                ...
                self._check_all_arguments_for_traversal(f"{tool_name}.{arg_name}", arg_value)
    ...
    def _verify_directory_access(self, tool_name: str, path_info: Any) -> None:
        ...
        if tool_name == "bash" and isinstance(path_info, dict) and "command" in path_info:
        ...
        if isinstance(path_info, str):
            ...
            if has_path_traversal_patterns(path):
                logger.warning(f"Path traversal pattern detected for {tool_name}: {path}")
                raise DirectoryTraversalError(
            ...
            try:
                ...
                if not abs_path.startswith(self.start_directory):
                    logger.warning(f"Directory traversal attempt detected for {tool_name}: {path}")
                    raise DirectoryTraversalError(
    ...
    def _get_permission_path(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        for arg_name, arg_value in arguments.items():
            if isinstance(arg_value, str) and arg_name in ("path", "file_path", "directory", "src", "dest", "source", "destination", "input", "output"):
                return arg_value
