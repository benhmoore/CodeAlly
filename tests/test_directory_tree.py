...
def test_generate_truncated_tree_basic(sample_directory_structure):
    ...
    assert os.path.basename(sample_directory_structure) in tree, "Root directory should be in the tree"
...
def test_generate_truncated_tree_depth_limit():
    ...
    assert os.path.basename(test_dir) in root_only_tree, "Root should still be in the tree"
    assert len(root_only_tree.strip().split('\n')) <= 2, "Root-only tree should be minimal"
...
def test_generate_truncated_tree_file_limit(sample_directory_structure):
    ...
    lines = tree.strip().split("\n")
    file_count = sum(1 for line in lines if not line.endswith("/") and not line.endswith("...") and "- " in line)
    
    # The tree should either have at most max_files files or include a truncation indicator
    assert file_count <= 2 or "..." in tree, "Tree should respect file limit or indicate truncation"
    ...
    file_entries = [line for line in lines if not line.endswith("/") and not line.endswith("...") and "- " in line]
    
    # Should either respect the limit or include truncation
    assert len(file_entries) <= 3 or "..." in tree_limited, "Should limit files or indicate truncation"
    
    # Test with a high file limit (should include all files)
    tree_high_limit = generate_truncated_tree(sample_directory_structure, max_files=1000)
...
def test_generate_truncated_tree_exclude_patterns(sample_directory_structure):
    ...
    tree = generate_truncated_tree(sample_directory_structure, exclude_patterns=exclude_patterns)
    ...
    tree_no_subdir = generate_truncated_tree(sample_directory_structure, exclude_patterns=["subdir*"])
    assert "subdir1" not in tree_no_subdir, "subdirectories matching the pattern should be excluded"
...
def test_get_gitignore_patterns(sample_directory_structure):
    ...
    assert ".DS_Store" in patterns, "The pattern .DS_Store should be extracted from .gitignore"
    ...
    assert isinstance(patterns, list), "Should return an empty list for directories without .gitignore"
    assert len(patterns) == 0, "Should return an empty list for directories without .gitignore"
    
    # Test with an empty .gitignore file
    empty_gitignore_path = os.path.join(sample_directory_structure, "dir1", ".gitignore")
    with open(empty_gitignore_path, "w") as f:
        pass  # Create empty file
    
    patterns = get_gitignore_patterns(os.path.join(sample_directory_structure, "dir1"))
    assert isinstance(patterns, list), "Should return an empty list for empty .gitignore file"
...
def test_generate_truncated_tree_edge_cases(temp_directory):
    ...
    assert tree.strip() == f"- {os.path.basename(empty_dir)}/", "Empty directory should only show the root"
